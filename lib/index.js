...
function getWholeRange(sourceCode, node) {
...
const rule$2 = {
    ...
    create: (context) => ({
        Program: (node) => {
            checkProgram$2(context, node);
        },
    }),
};
function checkProgram$2(context, program) {
...
function getGroup(packages, importType, importSource) {
...
function getEmptyLinesBetweenTokens(sourceCode, leftRange, rightRange) {
    const lastToken = sourceCode.getLastToken({ range: leftRange }, {
        includeComments: true,
    });
    const firstToken = sourceCode.getFirstToken({ range: rightRange }, {
        includeComments: true,
    });
    ...
    for (let index = 0; index < statementsWithPaths.length; ++index) {
        ...
        if (expected !== actual) {
            ...
            context.report({
                ...
                fix: (fixer) => [
                    fixer.removeRange(range),
                    fixer.insertTextBeforeRange(getWholeRange(sourceCode, actual).withWhitespace, getTextFromRange(sourceCode, range)),
                ],
            });
            return;
        }
